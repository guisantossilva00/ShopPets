...
//imports pubsub comunica de product detail para modal
...
export default class ProductDetails extends LightningElement {
    ...
    get displayableProduct() {
        // console.log('THIS:::', this.product.data);
        return {
            ...
            vendidoPor: this.product.data.fields.VendidoPor__c,
     ...
     * @private
     */
     triggerAddToCart(event) {
        console.log('clicou no add to cart');
        ...
        .then((result) => {
            ...
            console.log('vamos verificarse o tipo é igual')

            isSameType({cartId : this.cartId,  productId : this.recordId})
            .then((result) => {

                if(result) {
                    console.log('é do mesmo tipo adicione ao cart')
                    ...
                    .then((result) => {
                        ...
                        this.template.querySelector('c-modal-add-to-cart-custom[c-productdetails_productdetails]').openModal();
                    })
                    .catch((error) => {
                        this.dispatchEvent(
                            new ShowToastEvent({
                                title: 'Error',
                                message:
                                    '{0} não foi possível adicionar seu produto ao carrinho neste momento. Por favor, tente novamente mais tarde.',
                                messageData: [this.displayableProductdisplayableProduct.name],
                                variant: 'error',
                                mode: 'dismissable'
                            })
                        );
                    ...
                    console.log('é diferente não adicione e mostre erro')

                    this.template.querySelector('c-modal-error-custom[c-productdetails_productdetails]').openModal();
                }
            })
        })
        .catch((error) => {
     ...
     */
    handleModal(event) {
        console.log('handle modal', event);
         
        let productData = {'data': this.product.data, 'quantity': event.detail.quantity };
        
        console.log('pubsub: passando o product data para o modal')
        fireEvent(this.pageRef, 'sendtomodal', productData);
         
         
        console.log('Abrindo o modal');
        this.template.querySelector('c-modal-wishlist[c-productdetails_productdetails]').openModal();


        // console.log('clicou no add to list', this.product.data)
        ...
        // })
        // .then((result) => {
        //     console.log('result add to list', result);
