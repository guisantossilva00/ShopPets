...
export default class SearchResults extends NavigationMixin(LightningElement) {
    ...
    get headerText() {
        ...
        if (totalItemCount > 1) {
            ...
            text = `${startIndex} - ${endIndex} de ${totalItemCount} itens`;
    ...
    handleAction(evt) {
        evt.stopPropagation();
        this.productId = evt.detail.productId;
            ...
            if(result){
                ...
                .then((result) => {
                    ...
                    if(result) {
                        ...
                        .then(() => {
                            ...
                            this.template.querySelector('c-modal-add-to-cart-custom[c-searchresults_searchresults]').openModal();
                        })
                        .catch(() => {
                            this.dispatchEvent(
                                new ShowToastEvent({
                                    title: 'Error',
                                    message:
                                        '{0} não foi possível adicionar ao seu carrinho neste momento. Por favor, tente novamente mais tarde.',
                        ...
                        this.template.querySelector('c-modal-error-custom[c-searchresults_searchresults]').openModal();
                    }
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error',
                        message:
                            '{0} Não localizamos o id do carrinho. Por favor, tente novamente mais tarde.',
                        messageData: [evt.detail.productName],
                        variant: 'error',
                        mode: 'dismissable'
                    })
        ...
        .catch((error) => {
            this.dispatchEvent(
                new ShowToastEvent({
                    title: 'Error',
                    message:
                        '{0} Houve um problema para encontrar o ID do carrinho. Por favor, tente novamente mais tarde.',
                    messageData: [evt.detail.productName],
                    variant: 'error',
                    mode: 'dismissable'
                })
    ...
    handleQuickAction() {
        ...
        // send message to quick action
        let message = this.displayData;
        fireEvent(this.pageRef, 'sendtoquickaction', message);

        this.template.querySelector('c-quick-action[c-searchresults_searchresults]').openQuickAction();
    }
}